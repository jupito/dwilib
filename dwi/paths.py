...
def pmap_path(mode, case=None, scan=None, new=False):
    ...
    path = dwi.util.sglob('dicoms_{m[1]}_*'.format(m=mode), typ='dir')
    if case is not None and scan is not None:
        if mode[2] == 'raw':
...
def mask_path(mode, masktype, case, scan, lesion=None, algparams=()):
    ...
    if masktype == 'prostate':
        path = 'masks_{mt}_{m[0]}/{c}_*_{s}*'
    elif masktype == 'lesion':
        path = 'masks_{mt}_{m[0]}/PCa_masks_{m[0]}_{l}*/{c}_*{s}_*'
...
def texture_path(mode, case, scan, lesion, masktype, slices, portion, method,
    ...
    path = 'texture/texture_{mt}_{m[1]}_{m[2]}_{slices}_{portion}_{vx}'
    if voxel == 'mean':
        ext = 'txt'
    else:
        ext = 'h5'
